...
def config_params():
    ...
    n_patches=20 # Number of patches of each image which is randomly created.
    ...
    scales=[0.8, 1.2] # Scale patches with these scales. Used for augmentation.
    flip_index=[0,1,-1] # Flip image. Used for augmentation.


@ex.automain
def run(n_classes,n_epochs,input_height,
        input_width,weight_decay,n_patches,
        n_batch,patches,augmentation,flip_aug,
        elastic_aug,blur_aug,scaling,
         binarization,blur_k,scales,dir_train,
    ...
    provide_patches(dir_img_val,dir_seg_val,dir_flow_eval_imgs,
                    ...
                    input_height,input_width,blur_k,blur_aug,n_patches,
                    flip_aug,binarization,elastic_aug,scaling,scales,flip_index,
                    augmentation=False,patches=True)
